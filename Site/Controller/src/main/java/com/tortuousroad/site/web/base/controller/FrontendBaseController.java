...
import com.tortuousroad.framework.util.EmailUtil;
import com.tortuousroad.framework.util.EncryptionUtil;
import com.tortuousroad.framework.util.RegexUtil;
import com.tortuousroad.framework.util.SMSUtil;
import com.tortuousroad.framework.web.filter.controller.BaseController;
...
public class FrontendBaseController extends BaseController {
	 ...
	 *            电话号码
	 * @param action
	 *            目的，如：register(注册)、retrievePwd(找回密码)等，详见：com.tortuousroad.framework.util.SMSUtil.ACTION_MAP
	 * @param sender
	 *            发送者，site、app等
	 *            
	 * @return
	...
	public boolean checkSMSCode(String mobilephone, String smsCode) {
		if (StringUtils.isEmpty(mobilephone) || !RegexUtil.isValidPhoneNumberCN(mobilephone) || StringUtils.isEmpty(smsCode)) {
			return false;
		}
